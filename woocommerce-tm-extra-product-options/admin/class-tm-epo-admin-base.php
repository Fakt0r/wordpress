					...
					foreach ( $epos as $key => $epo ) {
						...
						if ( isset( $items['tm_item_id'] ) && isset( $items['tm_key'] ) && $items['tm_key'] == $key && $items['tm_item_id'] == $item_id ) {
							...
							if ( isset( $epo['price'] ) ) {
								...
								if ( !$new_currency ) {
									$epo['price'] = apply_filters( 'wc_epo_remove_current_currency_price', $epo['price'],TM_EPO()->get_element_price_type($saved_epos[ $key ]), NULL, $order_currency, $_current_currency_prices, isset($saved_epos[ $key ]['key'])?$saved_epos[ $key ]['key']:NULL );
								} 
