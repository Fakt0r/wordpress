	...
	function tm_init_epo( main_product, is_quickview, product_id, epo_id ) {
				...
				if ( ! $cart.data( 'tm_rules_init_done' ) ) {
					...
					$cart.find( '.tmcp-attributes, .tmcp-elements' ).each( function ( index, element ) {
						...
						if ( typeof rules === "object" ) {
							...
							var price = tm_apply_dpd( rules[ current_variation ], $totals, apply_dpd ),
								formatted_price = tm_set_price( price, $totals ),
								original_price = tm_apply_dpd( original_rules[ current_variation ], $totals, apply_dpd ),
		...
		function tc_add_dimensions(){
			main_product.find( '.tm-variation-ul-color .tmhexcolorimage-li-nowh,.tm-variation-ul-image .tmhexcolorimage-li-nowh,ul.use_images_container .tmhexcolorimage-li-nowh,ul.use_colors_container .tmhexcolorimage-li-nowh' ).each( function ( i, el ) {
				el = $( el );
				var eh = el.height(),ew = el.width(),
					w = (eh>ew?ew:eh) * 0.9, im = el.find( ".tmhexcolorimage" );
