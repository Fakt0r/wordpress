...
final class TM_EPO_COMPATIBILITY_woocommerce_currency_switcher {
	...
	/** WooCommerce Currency Switcher support (realmag777) **/
	public function wc_epo_remove_current_currency_price( $price = "", $type = "", $from_currency=NULL, $to_currency=NULL, $currencies = NULL, $key = NULL, $attribute = NULL) {
		global $woocommerce_wpml;
		if ( class_exists( 'WOOCS' ) ) {
			global $WOOCS;
			$currencies = is_callable( array( $WOOCS, 'get_currencies' ) )?$WOOCS->get_currencies():array();
			...
			$price = $this->get_price_in_currency( $price, $from_currency, $to_currency, $currencies, $type, $key, $attribute );
		}
